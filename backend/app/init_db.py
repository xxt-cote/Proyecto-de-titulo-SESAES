...
def init_db():
    ...
    # ── USUARIOS ──────────────────────────────────────────────
    # Se crean solo si la tabla está vacía; si ya existen usuarios
    # (p.ej. cargados desde SQL o previa ejecución) no se duplican.
    ...
    # Se crean independientemente de los usuarios: si la tabla está vacía
    # (aunque los usuarios ya existan), se insertan los registros base.
    # Esto evita que un login de profesional falle con "no encontrado" cuando
    # la base tiene usuarios pero la tabla profesional está vacía.
    ...
    if db.query(HorarioDisponible).count() == 0:
        horarios = [
